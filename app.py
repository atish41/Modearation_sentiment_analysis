...
from main import get_category_ids
# from app_v1 import get_category_ids
# import urllib3
    ...
    for url in urls:
        ...
        if ext in image_extensions:
            ...
            if any(pattern in url.lower() for pattern in ['img', 'image', 'photo', 'pic']):
                ...
                # Default to image if can't determine (can be adjusted based on needs)
                image_urls.append(url)
...
vertexai.init(project="noble-syntax-443010-j3", location="us-central1")
...
# System instruction and generation config remain the same
...
def fetch_and_preprocess_image(image_path):
    ...
    try:
        if image_path.startswith("http://") or image_path.startswith("https://"):
            response = requests.get(image_path, stream=True, headers=headers, verify=False)  # Set headers for browser mimicry
            if response.status_code != 200:
                raise ValueError(f"Failed to fetch image. Status code: {response.status_code}")

            image = Image.open(BytesIO(response.content))
            if image.format != 'JPEG':
                image = image.convert("RGB")  # Convert to RGB if not in JPEG format
...
#     keys = []
#     for value in values:
#         for key, val in CATEGORIES_LIST.items():
#             if val == value:
#                 keys.append(key)
#     return keys
...
#         category_ids.append(CATEGORIES_LIST[category])
#     return category_ids
...
def predict_categories(urls, caption=""):
    ...
    contents = [Part.from_text(caption)] if caption else []
...
def predict():
    ...
    try:
        ...
        # predicted_categories_dict={"predicted_categories":predicted_categories}
        print(predicted_categories)
        predicted_category_ids = get_category_ids(predicted_categories)
        print(predicted_categories_ids)
        # print(predicted_category_ids)
        return jsonify({
...
# def predict():
#     """
#     Flask route to handle category prediction requests.

#     Expects a JSON payload with:
#     - 'urls': A list of image or video URLs
#     - 'caption': An optional caption text (default: empty string)

#     Returns:
#         JSON response with predicted categories.
#     """
#     try:
...
#         # Predict categories
#         categories = predict_categories(urls, caption)

#         # Map categories to IDs
#         category_ids = get_category_ids(categories)

#         return jsonify({"categories": categories, "category_ids": category_ids}), 200
#     except Exception as e:
#         return jsonify({"error": str(e)}), 500
