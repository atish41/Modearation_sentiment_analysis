...
def get_category_ids(input_data: Union[str, Dict], threshold: int = 80) -> List[int]:
    """
    Get category IDs from JSON input containing predicted categories using fuzzy matching.

    Args:
        input_data: Either a JSON string or dictionary with 'predicted_categories' key.
        threshold: Minimum matching score for fuzzy matching.
    ...
    # Fuzzy match predicted categories to CATEGORIES
    ids = []
    for name in predicted_categories:
        match, score = process.extractOne(name.strip(), category_names.values(), scorer=fuzz.token_sort_ratio)
        if score >= threshold:
            # Get the corresponding ID for the matched category name
            matched_id = next((cat_id for cat_id, cat_name in category_names.items() if cat_name == match), None)
            if matched_id:
...
if __name__ == "__main__":
    ...
    print(ids)  # Output may vary based on fuzzy matching.
    ...
    ids = get_category_ids(input_dict)
    print(ids)